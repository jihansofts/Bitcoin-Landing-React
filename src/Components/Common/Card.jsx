...
const Card = ({ data }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-10 px-4">
      {Course.map((item) => (
        <div key={item.id} className="col-span-1 p-4 relative">
          {/* Card Image */}
          <img
          ...
          <div className="relative bg-bgSecondary flex flex-col items-start text-start py-6 px-5 sm:py-8 sm:px-6 md:py-10 md:px-7 rounded-b-2xl font-Inter font-semibold overflow-hidden">
            ...
            <div className="relative z-10">
              <h3 className="text-white text-[24px] max-xl:text-[20px] max-lg:text-[24px] max-sm:text-[28px] max-md:text-[32px] font-Inter font-semibold">
                {item.title}
              </h3>
              ...
              <button className="text-[14px] sm:text-[16px] md:text-[18px] mt-6 sm:mt-8 cursor-pointer w-full bg-buttonColor py-2 sm:py-3 px-5 sm:px-7 rounded-3xl font-Inter font-bold text-bgPrimary hover:bg-opacity-90 transition-all">
                Enroll Now
              </button>
            </div>
          </div>
